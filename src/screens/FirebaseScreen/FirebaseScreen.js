...
const FirebaseScreen = () => {
  ...
  const optimizedFn = useCallback(debounce(searchGroceryItem), []);
  ...
  const emptyList = () => {
    return (
      <EmptyListButton
        mainLabel="This gorcery item is not in your list."
        subLabel="Do you want to add this item in your list."
        icon={Images.order_failed}
        containerStyle={{marginTop: hp('15%')}}
        onPressYes={() => addGroceryOnPressYes()}
        onPressNo={() => getGroceryOnPressNo()}
      />
    );
  };
